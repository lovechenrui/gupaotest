...
public class GPProxy {
	...
	public static Object newProxyInstance(GPClassLoad classLoad,Class<?>[] interfaces,GPInvocationHandler h){
		try{
			 ...
			 * 1、动态生成源文件
			 */
			String srcStr = generateSrc(interfaces);
			/**
			 * 2、源文件写到磁盘
			 */
...
//			manager.close();
			...
			Iterable compitionfile = fileManager.getJavaFileObjectsFromStrings(Arrays.asList(filePath+"$Proxy0.java"));
...
//			f.delete();
	...
	private static String generateJava(Class<?>[] interfaces) {
		...
		sb.append("package com.chenrui.partten.proxy.jdk.custom;"+ln);
	...
	private static String generateSrc(Class<?>[] interfaces){
		...
		for (Method m : interfaces[0].getMethods()){
			...
			sb.append("Object rs = this.h.invoke(this,m,new Object[]{" + paramValues + "});" + ln);
			sb.append("System.out.println(rs);" + ln);
			sb.append((hasReturnValue(m.getReturnType()) ? "return " : "") +
					getCaseCode("rs",m.getReturnType()) + ";" + ln);
